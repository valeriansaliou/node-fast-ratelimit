...
var FastRateLimit = function(options) {
  ...
  this.__last_timestamp = Date.now();
};
 ...
 * @returns {object} A configured bucket status checking function, does not consume token by default
 */
var bucketCheck = function(consumeToken) {
  return function(namespace) {

    if (!namespace) {
      return { timeRemaining: 0,  tokens: 1 };
    }
    ...
    if (consumeToken) {
      this.__tokens.set(namespace, (_tokens_count - 1));
    }

    let timeRemaining = this.__last_timestamp + this.__options.ttl_millisec - Date.now();

    return { timeRemaining: timeRemaining, tokens: this.__tokens.get(namespace) };
  };
};
...
/**
 * FastRateLimit.prototype.bucketCheckSync
 * @public
 * @param  {string}  namespace
 * @return {object}  Remaining time and tokens in bucket
 */
FastRateLimit.prototype.bucketCheckSync = bucketCheck(false);
...
/**
 * FastRateLimit.prototype.bucketCheck
 * @public
 * @param  {string} namespace
 * @return {object} Promise object
 */
FastRateLimit.prototype.bucketCheck = function(namespace) {
  if (this.bucketCheckSync(namespace) === true) {
